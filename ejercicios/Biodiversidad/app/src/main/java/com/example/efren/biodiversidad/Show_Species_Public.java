...
public class Show_Species_Public extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_show__species__public);
        new Especies().execute();
    }

    private class Especies extends AsyncTask<String, String, ArrayList<Obj_Species_for_user>> {

        @Override
        protected ArrayList<Obj_Species_for_user> doInBackground(String... strings) {
            try {
                JSONArray jsonArray = new JSONArray((new Read_Page().Read_Page("http://biodiversidad.alcohomeapp.com.mx/webService_especies.php")));
                JSONObject jsonObject;
                for (int i = 0; i < jsonArray.length(); i++){
                    jsonObject = jsonArray.getJSONObject(i);
                    Obj_Species_for_user Obj_class = new Obj_Species_for_user();
                    Obj_class.setId_especie(jsonObject.getInt("id"));
        ...
        protected void onPostExecute(final ArrayList<Obj_Species_for_user> s) {
            ...
            if (s != null) {
                ...
                mLayoutManager = new LinearLayoutManager(Show_Species_Public.this);
                mAdapter = new MyAdapter_RecyclerView(Show_Species_Public.this, s, R.layout.recycler_view_item, new MyAdapter_RecyclerView.OnItemClickListener() {
                    @Override
                    public void onItemClick(Obj_Species_for_user name, int position) {
                        Intent intent = new Intent(Show_Species_Public.this, Show_Information.class);
                ...
                Toast.makeText(Show_Species_Public.this, "Error al cargar", Toast.LENGTH_SHORT).show();
            }
            }
    }
}